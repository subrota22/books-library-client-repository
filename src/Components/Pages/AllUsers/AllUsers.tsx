...
const AllUsers = () => {
    ...
    const handleSearchFeild = (searchText: any) => {
        const findData: any = data?.filter((bookData: any) => bookData?.name?.toLowerCase().includes(searchText?.toLowerCase()));
        ...
        <>
          ...
          <i className="fa-solid fa-magnifying-glass text-4xl ml-2 mr-6 btn btn-primary pb-7 pt-1 px-3"></i>
            </div>
            {
                ...
                <h2 className="text-5xl font-extrabold text-info text-center h-screen mt-40"> Users not found </h2>
            }
                            ...
                            data?.map((book: any) =>
                                <div className="card w-96 ourUserCard  h-auto bg-base-100 shadow-xl" key={book?._id}  data-aos="zoom-in">
                                    ...
                                    <div className="card-body">
                                        <h2 className="card-title"> <span className="text-info">Name : </span> {book?.name ? book?.name : " name not found"}</h2>
                                        <h2 className="card-title"><span className="text-info"> Email : </span>{book?.email ? book?.email : "author not found"}</h2>
                                    </div>
                                </div>
                            )
                        }
